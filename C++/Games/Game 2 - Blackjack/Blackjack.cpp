...
// Displays the rules of the game only
void displayRules()
{
...
// Displays the menu for the game only
void displayMenu()
{
...
void play(vector<player> &players)
{
    ...
    // Shows each players score but the dealers
	for(int i=1;i<players.size();i++)
    {
		cout << players[i].info.username << " has: " << score(players[i].hand) << setw(10) << endl;
	}

	// Displays each persons cards
	...
	bool cont = true; 
	for(int i = 1;i<players.size();i++){ // Cycle through all the players since each player plays on their own
		do{
            ...
            { 
                ...
                { 
                    ...
                    // Cycles through the players and pays them out since they lost
					for(int i =1;i<players.size();i++)
                    {
            ...
            { 
                ...
                { 
                    ...
                    // Pays out the players since they just lost
					for(int i =1;i<players.size();i++)
                    {
						payout(players[0],players[i]);
					}
			...
			if(score(players[0].hand) <= 21){
					...
					switch(input)
                    {
                        case 'L': //they wanted to split
                            split(players[0], players[i]); // split them
                        ...
                        case 'D':
                            ...
                            break;
                        case 'H':
                            players[i].hand.push_back(hitMe()); // give them one more card for their hit
		...
		}while(!(input == 'S' || input == 's')); // continue doing this until they are want to stay
	}

	dealer_play(players[0]); //now the dealer plays

	players[0].hand[0].up = true; // everybody can see the first card
	...
	for(int i =1;i<players.size();i++)
    {
		...
		int win = winner(players[0], players[i]); // figure out who wins
		if(win == 1)
        {
			players[i].info.wins += 1; //if the player wins, add one to their win record
		}
		payout(players[0],players[i]); // payout everybody
		clear(players[i].hand); // clear out their hands
...
account create(){
	...
	// Checks to see if a username is available by checking to see if the file can even be open
	do
    {
...
void save(player user)
{
	...
	string filename = user.info.username + ".dat"; //what file to save to

	output.open(filename, ios::out); // erase all previous data
...
account load()
{
	string username, password; // know what the username and password is
	string filename; 
	account user; //sets up an account to take in the data
...
// Returns the Character version of the suit (takes in a number between 0-4 and returns the suit based off that number)
...
// Gets the score for the user (treat Aces initially as 1 and then later check to see if the hand contains an Ace)
...
card deal()
{
	...
	new_card.up = true; // change it later if it's the dealers face down card
