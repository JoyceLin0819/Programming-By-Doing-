...
int main()
{
	...
	do
	{
        ...
        { 
			...
			m.DisplayGrid();  
			m.Player2_Choice(); 
		}while (m.again);  // Until Player 2 chooses to quit
...
void Maxit :: InitialGrid()  // Initializes a random grid 
{
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < n; j++)
		{
			do
			{
				temp = -9 + rand() % 25; // Num range between -9 to 15
...
void Maxit::Player1_Choice()  // Record and update Player 1 choice and scores
{
	...
	if (flag)
	{
		...
		if (!stop)       // Executes if Player 1 hasn't quit
		{
			...
			a[c_row][c_col] = -100;           // Update num as taken
